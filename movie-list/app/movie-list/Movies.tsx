...
import { MovieList, getPopularMovies, addFavoriteMovie } from "@/utils/service";
...
export default function MoviesPage() {
    ...
    async function addFavorite(movieId: number) {
        console.log("add favorite: ", movieId)
        const res = await addFavoriteMovie(movieId)

        console.log("add favorite res: ", res)
    }
                    ...
                    {data?.results.map((movie) => (
                        <div key={movie.id} className={styles.movie_item}>
                            ...
                            <div style={{ marginBottom: "10px" }} className={styles.flex90}>
                                ...
                                <svg onClick={() => addFavorite(movie.id)}
                                    fill="white" width="40px" height="40px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
